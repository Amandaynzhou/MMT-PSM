...
**************************************************************************/
/*
2020/02/01 by yanning zhou
/* add function to reture iou, intersection, union simultaniously by Zhou Yanning*/
*/
#include "maskApi.h"
#include <math.h>
#include <stdlib.h>
...
/*adding------------*/
/* add function to reture iou, intersection, union simultaniously by Zhou Yanning*/
void rleIouInterUnion( RLE *dt, RLE *gt, siz m, siz n, byte *iscrowd, double *o, double *inter, double *un ) {
  siz g, d; BB db, gb; int crowd;
  db=malloc(sizeof(double)*m*4); rleToBbox(dt,db,m);
  gb=malloc(sizeof(double)*n*4); rleToBbox(gt,gb,n);
  bbIou(db,gb,m,n,iscrowd,o); free(db); free(gb);
