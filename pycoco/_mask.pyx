...
2020/02/01 by yanning zhou
 add func iouIntUni by Zhou Yanning, return iou, intersection, union simultaniously.
'''
...
def iouIntUni( dt, gt, pyiscrowd ):
    def _preproc(objs):
        ...
        if type(objs) == np.ndarray:
            ...
            raise Exception('unrecognized type.  The following type: RLEs (rle), np.ndarray (box), and list (box) are supported.')
        return objs
    def _rleIou(RLEs dt, RLEs gt, np.ndarray[np.uint8_t, ndim=1] iscrowd, siz m, siz n, np.ndarray[np.double_t,  ndim=1] _iou, np.ndarray[np.double_t,  ndim=1] _inter, np.ndarray[np.double_t,  ndim=1] _un):
        rleIouInterUnion( <RLE*> dt._R, <RLE*> gt._R, m, n, <byte*> iscrowd.data, <double*> _iou.data, <double*> _inter.data, <double*> _un.data )
        #rleIou( <RLE*> dt._R, <RLE*> gt._R, m, n, <byte*> iscrowd.data, <double*> _iou.data )
    def _bbIou(np.ndarray[np.double_t, ndim=2] dt, np.ndarray[np.double_t, ndim=2] gt, np.ndarray[np.uint8_t, ndim=1] iscrowd, siz m, siz n, np.ndarray[np.double_t, ndim=1] _iou):
        bbIou( <BB> dt.data, <BB> gt.data, m, n, <byte*> iscrowd.data, <double*>_iou.data )
    def _len(obj):
    ...
    # define local variables
    cdef double* _iou = <double*> 0
    cdef double* _inter = <double*> 0
    cdef double* _un = <double*> 0
    cdef np.npy_intp shape[1]
    # check type and assign iou function
    if type(dt) == RLEs:
        _iouFun = _rleIou
        ...
        raise Exception('input data type not allowed.')
    _iou = <double*> malloc(m*n* sizeof(double))
    _inter = <double*> malloc(m*n* sizeof(double))
    _un = <double*> malloc(m*n* sizeof(double))
    iou = np.zeros((m*n, ), dtype=np.double)
    inter = np.zeros((m*n, ), dtype=np.double)
    union = np.zeros((m*n, ), dtype=np.double)
    shape[0] = <np.npy_intp> m*n
    iou = np.PyArray_SimpleNewFromData(1, shape, np.NPY_DOUBLE, _iou)
    inter = np.PyArray_SimpleNewFromData(1, shape, np.NPY_DOUBLE, _inter)
    union = np.PyArray_SimpleNewFromData(1, shape, np.NPY_DOUBLE, _un)
    PyArray_ENABLEFLAGS(iou, np.NPY_OWNDATA)
    PyArray_ENABLEFLAGS(inter, np.NPY_OWNDATA)
    PyArray_ENABLEFLAGS(union, np.NPY_OWNDATA)
    _iouFun(dt, gt, iscrowd, m, n, iou, inter, union)
    return iou.reshape((m,n), order='F'), inter.reshape((m,n), order='F'), union.reshape((m,n), order='F')
