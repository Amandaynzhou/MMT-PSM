...
class RPNModule(torch.nn.Module):
    ...
    def _forward_single_head(self,features, i):
        objectness, rpn_box_regression = self.head(features)
        # pdb.set_trace()
        if i%2!=0:
    ...
    def forward_teacher(self, images, features, targets = None):
        if isinstance(features, list):
        ...
        with torch.no_grad():
            ...
            objectness, box_regression, sample_idx,label = \
                self.loss_evaluator.teacher_sample_selection(
                    anchors,objectness,rpn_box_regression,targets)
...
def build_rpn(cfg, is_teacher = False):
    """
