...
class MaskPostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        # np.save(os.path.join('mask_feature', ''.join(random.sample(string.ascii_letters + string.digits, 8)) + '.npy'),
        #         mask_prob.cpu().numpy())
...
def crop_mask_to_proposal(mask, proposal, box, im_h, im_w,
                          thresh=0.5,
                          padding=1):
    ...
    proposal = proposal.to(dtype=torch.int32)
    TO_REMOVE = 1
    ...
    im_mask = im_mask[proposal[1]:proposal[3],
              proposal[0]:proposal[2]]
    im_mask = im_mask.expand((1, 1, -1, -1))
    im_mask = F.interpolate(im_mask,size= (M,M),mode='bilinear', align_corners=False)
    im_mask = im_mask[0][0]
...
def make_roi_mask_generator(cfg):
    mask_threshold = cfg.MODEL.ROI_MASK_HEAD.POSTPROCESS_MASKS_THRESHOLD
    masker = Masker(threshold=mask_threshold, padding=1)
    mask_post_processor = MaskPostProcessor(masker)
    return mask_post_processor

def mask_mutual_target_processor(cfg):
    mask_threshold = cfg.MODEL.ROI_MASK_HEAD.POSTPROCESS_MASKS_THRESHOLD
    masker = Masker(threshold=mask_threshold, padding=1)
    return masker